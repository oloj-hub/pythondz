...
def rocket():
    Window.create_rectangle(130 + rocket_S_x, 260 + rocket_S_y, 85 + rocket_S_x, 90 + rocket_S_y, fill="grey",
                            outline="#646464")
    Window.create_polygon((55 + rocket_S_x, 290 + rocket_S_y), (160 + rocket_S_x, 290 + rocket_S_y),
                          (130 + rocket_S_x, 260 + rocket_S_y), (85 + rocket_S_x, 260 + rocket_S_y), fill="grey",
                          outline="#646464")
    ...
    Window.create_polygon((85 + rocket_S_x, 90 + rocket_S_y), (130 + rocket_S_x, 90 + rocket_S_y),
                          (107.5 + rocket_S_x, 55 + rocket_S_y), fill="grey", outline="black")
...
def shoot(pressed_button):
    global bullet_ident
    x = Window.create_line((bullet_x0 - 117.5, bullet_y0 - 42.5), (bullet_x0 - 107.5, bullet_y0 - 42.5), width=5,
                           fill="green")
...
def bullets():
    ...
    for i in range(0, bullet_ident):
        ...
        x = Window.create_line((bullet_list_x[2 * (i - 1)], bullet_list_y[i - 1]),
                               (bullet_list_x[2 * (i - 1) + 1], bullet_list_y[i - 1]), width=5, fill="green")
        bullet_list[i - 1] = x
...
def process():
    ...
    if q == 211:
        for i in range(10, (d - 490) * 10, 10):
            Window.create_oval(
                (math.fabs(130 + rocket_S_x + d / 3 - 30 - i), math.fabs(290 - i + rocket_S_y + d / 3 - 30)),
                (math.fabs(85 + rocket_S_x - d / 3 - 30 + i), math.fabs(190 + rocket_S_y + i - d / 3 - 30)), fill='red')
        d += 1
